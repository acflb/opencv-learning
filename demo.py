...
def capture_window_roi(window_title, relative_box=None):
    ...
    if not windows:
        raise ValueError(f"未找到标题包含 '{window_title}' 的窗口！请检查窗口是否已打开。")
    ...
    print(f"找到窗口 '{win.title}' -> 屏幕位置: X={win_x}, Y={win_y}, 宽度={win_w}, 高度={win_h}")
...
if __name__ == "__main__":
    ...
    try:
        print("正在获取窗口截图...")
        roi_img, abs_box = capture_window_roi(TARGET_WINDOW_TITLE, relative_box=RELATIVE_BOX)
